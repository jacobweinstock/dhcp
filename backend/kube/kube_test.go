	...
	tests := map[string]struct {
		...
		"one bad nameserver": {
			...
			want: &data.DHCP{
				...
				DefaultGateway: []byte{192, 168, 2, 1},
				NameServers:    []net.IP{net.IPv4(1, 1, 1, 1)},
				IPAddress:      []byte{192, 168, 2, 4},
...
func TestRead(t *testing.T) {
	tests := map[string]struct {
		...
		"good data": {hwObject: []v1alpha1.Hardware{hwObject1}, wantDHCP: &data.DHCP{
			...
			IPAddress:      []byte{172, 16, 10, 100},
			SubnetMask:     []byte{0xff, 0xff, 0xff, 0x00},
			DefaultGateway: []byte{172, 16, 10, 1},
			NameServers: []net.IP{
				{0x1, 0x1, 0x1, 0x1},
			},
			Hostname:  "sm01",
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if diff := cmp.Diff(gotDHCP, tc.wantDHCP); diff != "" {
				t.Fatal(diff)
			}
