...
func (h *Handler) setDHCPOpts(_ context.Context, _ *dhcpv4.DHCPv4, d *data.DHCP) []dhcpv4.Modifier {
	...
	if len(d.BroadcastAddress) > 0 && d.BroadcastAddress.IsUnspecified() {
	...
	if len(d.DefaultGateway) > 0 && d.DefaultGateway.IsUnspecified() {
